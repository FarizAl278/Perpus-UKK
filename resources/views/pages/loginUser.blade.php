<x-html>
    ...
    <div class="min-h-screen relative flex items-center justify-center px-4 py-12 overflow-hidden"
        ...
        {{-- Decorative Cloud Blobs (Soft) --}}
        <div class="absolute top-0 right-0 w-[500px] h-[500px] bg-white/60 rounded-full blur-3xl -z-10 translate-x-1/3 -translate-y-1/3"></div>
        <div class="absolute bottom-0 left-0 w-[400px] h-[400px] bg-sky-200/40 rounded-full blur-3xl -z-10 -translate-x-1/3 translate-y-1/3"></div>
        <div class="absolute top-1/2 left-1/2 w-72 h-72 bg-white/30 rounded-full blur-2xl -z-10 -translate-x-1/2 -translate-y-1/2"></div>

        {{-- LOGIN CARD --}}
        <div class="w-full max-w-md bg-white/80 backdrop-blur-xl border border-white/70 rounded-3xl shadow-[0_12px_40px_rgba(14,165,233,0.12)] p-8 sm:p-10 transform transition duration-300 hover:shadow-[0_16px_48px_rgba(14,165,233,0.18)]">
            
            {{-- HEADER --}}
            <div class="text-center mb-8">
                <div class="w-16 h-16 bg-gradient-to-br from-blue-50 to-blue-200 rounded-full flex items-center justify-center mx-auto mb-5 shadow-lg shadow-sky-300/40">
            ...
            <form method="POST" action="{{ route('siswa.login.store') }}">
                ...
                <div class="flex items-center justify-between mb-7">
                    <div class="flex items-center">
                        ...
                        <label for="remember" class="ml-2.5 text-sm text-slate-600 cursor-pointer select-none">Ingat saya</label>
                    </div>
                    <a href="#" class="text-xs text-sky-500 hover:text-sky-700 font-medium transition hover:underline">Lupa password?</a>
                </div>
            ...
            <div class="mt-8 pt-6 border-t border-slate-200/50 text-center">
                <p class="text-xs text-slate-400">
                    Belum punya akun? 
                    <a href="#" class="text-sky-500 hover:text-sky-700 font-medium transition hover:underline">Hubungi Petugas Perpustakaan</a>
                </p>
            </div>
        </div>
    </div>
